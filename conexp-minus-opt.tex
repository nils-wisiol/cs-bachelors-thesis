\chapter{A set in $\coNEXP \setminus \OPT$}
  ...
  Now, let us take a closer look at this set \(L\) that has no optimal proof system. One first observation is that the density of \(L_i\) is given by
    \[ \dens_{L_i}(n) = |\{ x \in \Sigma^*, x=0^i10^j, 0 \leq j \leq n - i - 1 \}| =
       \begin{cases}
        0 & (n < i), \\
        n - i & (n \geq i). \\
       \end{cases}
    \]
  From \(L_i' \subseteq L_i\) it follows that \(\dens_{L_i'} \leq \dens_{L_i}\) and so we can get a upper bound for the density of \(L\). Since the density of \(L_i\) is for all \(i > n\) zero, we get
    \[ \dens_L(n) = \dens_{\bigcup L_i'}(n) = \sum_{i=1}^\infty \dens_{L_i'}(n) \leq \sum_{i=1}^\infty \dens_{L_i}(n) = \sum_{i=1}^n n - i = n^2 + \frac{n}{2}. \]
  This yields

  \begin{lemma}
    With \(t(n)\) defined as above, there is a sparse language \(L \in \coNTIME(t(n))\) that has no p-optimal proof system.
  \end{lemma}
