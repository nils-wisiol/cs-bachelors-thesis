\chapter{A set in $\coNEXP \setminus \OPT$} \label{chpConexpMinusOpt}
  ...
  \begin{proof}[Formal proof of theorem \ref{thmMain}]
    ...
    We obtain this enumeration by enumerate all Turing transducers (using Gödel's enumeration) and combine each transducer with a clock that ensures a maximum runtime of \(n^i + i\) for a transducer \(M_i\).
