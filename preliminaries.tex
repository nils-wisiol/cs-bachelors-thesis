\chapter{Preliminaries}
  ...
  \begin{definition}
    A function \(h \in \FP\) is called \defNotion{proof system} for a language \(L\) if the range of \(h\) is \(L\). A string \(w\) with \(h(w) = x\) is called an \defNotion{\(h\)-proof} for \(x\). \(h\) is called an \defNotion{polynomially bounded} proof system, if there is a polynomial \(p\) such that for every \(x \in L\), there is a \(h\)-proof \(w\) with \(|w| \leq q(|x|)\).
  \end{definition}
